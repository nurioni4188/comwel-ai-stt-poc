...
- [ ] PR #5 최신 브랜치 `npm run build`
- [ ] PR #5 최신 브랜치 `npm run lint`
- [ ] 기존 extractive 요지 생성 회귀 없음
- [ ] 수정본 저장 시 `version_no` 증가
- [ ] 이전 버전 `status = superseded`, `is_current = false`
- [ ] 세션별 `is_current = true` 1건
- [ ] 확정 시 `status = confirmed`
- [ ] 확정 시 `confirmed_at is not null`
- [ ] 확정 후 textarea 읽기 전용
- [ ] 새 녹음 시작 시 편집 상태 초기화
- [ ] 자동 제출·자동 처분 없음 확인
