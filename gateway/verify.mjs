...
const start = normalizeTwilioMessage({
  event: 'start', sequenceNumber: '1', streamSid: 'MZ_TEST_STREAM',
  start: { streamSid: 'MZ_TEST_STREAM', callSid: 'CA_TEST_CALL', mediaFormat: { encoding: 'audio/x-mulaw', sampleRate: '8000', channels: '1' } },
});
...
assert.throws(() => normalizeTwilioMessage({
  event: 'start', sequenceNumber: '1', streamSid: 'MZ_BAD',
  start: { streamSid: 'MZ_BAD', callSid: 'CA_BAD', mediaFormat: { encoding: 'audio/pcm', sampleRate: '16000', channels: '1' } },
}), /unsupported Twilio media format/);

const mulaw = Buffer.alloc(160, 0xff);
assert.equal(mulaw8kToPcm16k(mulaw.toString('base64')).length, 640);
...
assert.equal(pcm24kToMulaw8k(Buffer.alloc(480)).length, 80);
...
assert.match(liveBridgeSource, /\/api\/telephony-adapter/);
assert.match(liveBridgeSource, /gatewayOperation/);
assert.match(liveBridgeSource, /'stt_ingest'/);
assert.match(liveBridgeSource, /'rag_answer'/);
assert.match(liveBridgeSource, /'session_complete'/);
...
const adapterSource = readFileSync(new URL('../api/telephony-adapter.ts', import.meta.url), 'utf8');
assert.match(adapterSource, /requireGatewayInternalAuth/);
assert.match(adapterSource, /GATEWAY_OPERATIONS/);
assert.match(adapterSource, /stt_ingest: '\/api\/stt-ingest'/);
assert.match(adapterSource, /rag_answer: '\/api\/stt-rag-answer'/);
assert.match(adapterSource, /session_complete: '\/api\/stt-session-complete'/);
assert.match(adapterSource, /process\.env\.VERCEL_URL/);
assert.doesNotMatch(adapterSource, /req\.headers\.host/);
...
console.log('PASS v0.15 gateway verification: Twilio contract, audio framing, protected controls, protected adapter upstream, STT lifecycle, TTS PCM contract');
